...
Mục tiêu tổng quát của đồ án là thiết kế, hiện thực và đánh giá một hệ thống SoC hoàn chỉnh trên nền tảng FPGA Kria KV260, có khả năng tăng tốc xử lý mạng nơ-ron nhân tạo. Hệ thống sẽ kết hợp sự linh hoạt của vi xử lý RISC-V với sức mạnh tính toán của phần cứng chuyên dụng.

Các mục tiêu cụ thể bao gồm:
\begin{itemize}
    \item Nghiên cứu kiến trúc Pulpino và RISC-V: Tìm hiểu sâu về kiến trúc hệ thống của Pulpino SoC, cách thức tổ chức lõi xử lý, bộ nhớ và các ngoại vi. Sử dụng Pulp làm cơ sở tham chiếu để triển khai hoặc tùy biến một hệ thống vi xử lý RISC-V phù hợp lên FPGA.
    \item Làm chủ chuẩn giao tiếp AXI4: Nghiên cứu đặc tả kỹ thuật của giao thức AMBA AXI4 (bao gồm AXI4-Lite cho điều khiển và AXI4-Full/Stream cho truyền dữ liệu). Mục tiêu là thiết kế được hệ thống bus liên kết (Interconnect) đảm bảo tính toàn vẹn dữ liệu và tối ưu hóa băng thông giữa CPU và bộ tăng tốc.
    \item Thiết kế bộ tăng tốc AI (AI Accelerator): Xây dựng khối phần cứng chuyên dụng (Hardware Accelerator) để thực thi các phép toán nhân-cộng (MAC) và tích chập (Convolution) của mạng CNN, giúp giảm tải cho vi xử lý trung tâm.
    \item Tích hợp và Đánh giá hệ thống: Tích hợp bộ tăng tốc vào hệ thống SoC thông qua giao tiếp AXI4. Kiểm chứng hoạt động của toàn bộ hệ thống bằng việc chạy mô hình phân lớp ảnh (Fashion-MNIST), đo đạc các thông số về độ chính xác, tốc độ thực thi và tài nguyên tiêu thụ trên FPGA.
\end{itemize}
...
Hiện nay, Edge AI trở thành xu hướng tất yếu trong hệ thống nhúng và IoT, do yêu cầu xử lý dữ liệu nhanh, giảm độ trễ và bảo vệ quyền riêng tư.
Trong khi đó, RISC-V nổi bật nhờ tính mở, linh hoạt, dễ mở rộng và đang được cộng đồng nghiên cứu rộng rãi để thay thế các kiến trúc độc quyền như ARM.
...
\begin{itemize}
    \item \textbf{Phạm vi triển khai phần cứng:} Đề tài tập trung vào việc mô phỏng, tích hợp và triển khai thực nghiệm lõi vi xử lý mềm (Soft-core) RISC-V trên nền tảng FPGA Xilinx Kria KV260. Hệ thống không hướng tới việc sản xuất chip ASIC thương mại mà dừng lại ở mức kiểm chứng trên FPGA.

    \item \textbf{Giới hạn về thiết kế vi kiến trúc:} Nhóm thực hiện không đi sâu vào thiết kế lại chi tiết vi kiến trúc (Micro-architecture) của lõi CPU RISC-V ở mức RTL (Register Transfer Level) từ con số không. Thay vào đó, đề tài sẽ tận dụng và tùy biến các nhân IP nguồn mở đã được kiểm chứng để tập trung nguồn lực vào việc thiết kế bộ tăng tốc AI (AI Accelerator) và hệ thống giao tiếp bus.

    \item \textbf{Phạm vi thuật toán và mô hình AI:} Việc xây dựng và huấn luyện mô hình (Training) được thực hiện Offline trên máy tính cá nhân sử dụng framework TensorFlow phổ biến  với kiến trúc mạng CNN (Convolutional Neural Network). Hệ thống trên FPGA chỉ đảm nhận nhiệm vụ suy luận dựa trên bộ trọng số đã được huấn luyện và lượng tử hóa.

    \item \textbf{Đánh giá hiệu năng:} Việc đánh giá hệ thống giới hạn ở các phép so sánh tương quan về tốc độ xử lý (Inference Time), thông lượng (Throughput) và tài nguyên tiêu thụ (LUT, FF, BRAM, Power) giữa các kịch bản: chạy thuần phần mềm trên CPU ARM (có sẵn trên Kria KV260), chạy trên lõi RISC-V mềm, và chạy trên hệ thống RISC-V có tích hợp bộ tăng tốc phần cứng.

    \item \textbf{Dữ liệu kiểm thử:} Để phù hợp với tài nguyên bộ nhớ giới hạn của hệ thống nhúng và tập trung vào kiểm chứng kiến trúc tính toán, dữ liệu đầu vào sẽ được giới hạn ở các bộ dữ liệu ảnh tiêu chuẩn có độ phân giải trung bình/thấp (như Fashion-MNIST hoặc CIFAR-10), chưa mở rộng sang xử lý video thời gian thực độ phân giải cao (HD/Full HD).
\end{itemize}
